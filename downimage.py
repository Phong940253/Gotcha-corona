    ...
    async def play_game(websocket, path):
        ...
        while True:
            ...
            if json_data["isLastWave"]:
                ...
                for ind, val in enumerate(listImage):
                    im_bytes = base64.b64decode(val)
                    # im_arr is one-dim Numpy array
                    im_arr = np.frombuffer(im_bytes, dtype=np.uint8)
                    image = cv.imdecode(im_arr, flags=cv.IMREAD_COLOR)
                    cv.imwrite("./Corona/sample/" +
                               listWave[ind] + ".jpg", image)
                # model.eval(listImage)
